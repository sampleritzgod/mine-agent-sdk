...
See the [repository root](../../README.md) for the full monorepo layout (`apps/web`, `examples/*`).
...
This package lives in a pnpm/Turborepo monorepo — see the [repository root](../../README.md) for the full setup. From the repo root:

```bash
pnpm install
pnpm --filter mine-agent-sdk test
pnpm --filter mine-agent-sdk build
```

## Examples

Each example is its own runnable package under [`examples/`](../../examples) at the repo root — run from there:

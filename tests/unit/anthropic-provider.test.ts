...
test("AnthropicProvider merges consecutive tool results into one user message with multiple tool_result blocks", async () => {
  ...
  const client = fakeClient(params => {
    captured = params;
    return { id: "msg_2", content: [{ type: "text", text: "done" }], stop_reason: "end_turn", usage: { input_tokens: 1, output_tokens: 1 } };
  });
...
test("AnthropicProvider maps user message images into content blocks (url and data URI)", async () => {
  ...
  const client = fakeClient(params => {
    captured = params;
    return { id: "msg_4", content: [{ type: "text", text: "ok" }], stop_reason: "end_turn", usage: { input_tokens: 1, output_tokens: 1 } };
  });
...
test("AnthropicProvider throws ConfigurationError when responseFormat name collides with a real tool", async () => {
  const client = fakeClient(() => ({ id: "x", content: [], stop_reason: "end_turn", usage: { input_tokens: 0, output_tokens: 0 } }));
  const provider = new AnthropicProvider({ model: "claude-3-5-sonnet-latest", client });
...
async function* fakeStreamEvents(): AsyncGenerator<Anthropic.Messages.RawMessageStreamEvent> {
  yield {
    ...
    message: {
      ...
      stop_sequence: null,
      type: "message",
      usage: { input_tokens: 3, output_tokens: 0, cache_creation_input_tokens: null, cache_read_input_tokens: null, server_tool_use: null, cache_creation: null, service_tier: null },
    },
  ...
  yield { type: "content_block_delta", index: 0, delta: { type: "text_delta", text: "Hel" } } as Anthropic.Messages.RawMessageStreamEvent;
  yield { type: "content_block_delta", index: 0, delta: { type: "text_delta", text: "lo" } } as Anthropic.Messages.RawMessageStreamEvent;
  ...
  yield { type: "content_block_delta", index: 1, delta: { type: "input_json_delta", partial_json: '{"a":' } } as Anthropic.Messages.RawMessageStreamEvent;
  yield { type: "content_block_delta", index: 1, delta: { type: "input_json_delta", partial_json: "1}" } } as Anthropic.Messages.RawMessageStreamEvent;
  yield { type: "content_block_stop", index: 1 } as Anthropic.Messages.RawMessageStreamEvent;
  yield {
    type: "message_delta",
    delta: { stop_reason: "tool_use", stop_sequence: null },
    usage: { input_tokens: null, output_tokens: 7, cache_creation_input_tokens: null, cache_read_input_tokens: null, server_tool_use: null },
  } as Anthropic.Messages.RawMessageStreamEvent;
...
test("AnthropicProvider.stream maps text and tool-use deltas, carrying id/name across fragments", async () => {
  ...
  const toolDeltas = chunks.filter(chunk => chunk.toolCallDelta).map(chunk => chunk.toolCallDelta!);
...
test("AnthropicProvider capability flags", () => {
  const provider = new AnthropicProvider({ model: "claude-3-5-sonnet-latest", client: fakeClient(() => ({})) });

...
export class ToolExecutor {
  ...
  private parseInput(tool: AnyTool, call: ToolCall): { success: true; input: unknown } | { success: false; error: unknown } {
    try {
      ...
      return {
        success: false,
        error: new ToolInputError(`Tool "${tool.name}" received malformed JSON arguments.`, {
          tool: tool.name,
        }, error),
      };
    }
  ...
  private async executeOnce(
    ...
    const timeout = new Promise<never>((_, reject) => {
      const handle = setTimeout(() => {
        controller.abort();
        reject(new ToolTimeoutError(`Tool "${tool.name}" timed out after ${timeoutMs}ms.`, {
          tool: tool.name,
          timeoutMs,
        }));
      }, timeoutMs);
  ...
  private retryDelay(backoff: number | ((attempt: number) => number) | undefined, attempt: number): number {
...
export function createTool<
  ...
  TResult,
>(tool: import("./tool").ToolDefinition<TSchema, TResult>): import("./tool").ToolDefinition<TSchema, TResult> {
  return tool;
}

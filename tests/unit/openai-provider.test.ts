...
test("OpenAIProvider maps user message images into multipart content", async () => {
  ...
  const client = fakeClient(params => {
    captured = params;
    return { id: "chatcmpl_3", choices: [{ message: { role: "assistant", content: "ok" }, finish_reason: "stop" }] };
  });
...
test("OpenAIProvider maps a json_schema responseFormat request", async () => {
  ...
  const client = fakeClient(params => {
    captured = params;
    return { id: "chatcmpl_4", choices: [{ message: { role: "assistant", content: "{}" }, finish_reason: "stop" }] };
  });
...
async function* fakeToolCallChunks() {
  ...
  yield {
    ...
    choices: [
      {
        index: 0,
        delta: { tool_calls: [{ index: 0, id: "call_1", function: { name: "add", arguments: '{"a":' } }] },
        finish_reason: null,
      },
    ],
  };
  ...
  yield { id: "c1", choices: [], usage: { prompt_tokens: 3, completion_tokens: 4, total_tokens: 7 } };
}

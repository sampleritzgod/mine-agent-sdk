...
function toAssistantContent(message: AssistantMessage): string | AnthropicContentBlockParam[] {
  ...
  for (const call of message.toolCalls) {
    blocks.push({ type: "tool_use", id: call.id, name: call.name, input: normalizeToolInput(call.arguments) });
  }
  return blocks;
}
...
function fromMessage(message: Anthropic.Messages.Message, structuredName: string | undefined): ModelResponse {
  let content = "";

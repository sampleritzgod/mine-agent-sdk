...
export class AgentRuntime {
  ...
  private async drive(
    generator: AsyncGenerator<AgentStreamEvent, RunResult, void>,
  ): Promise<RunResult> {
  ...
  private async *execute(
    ...
    try {
      ...
      while (state.iteration < this.maxIterations) {
        ...
        if (response.handoff) {
          const output = await this.performHandoff(
            response.handoff,
            runId,
            metadata,
            tracer,
  ...
  private *finish(
    result: RunResult,
    streaming: boolean,
  ): Generator<AgentStreamEvent, RunResult, void> {
    if (streaming) {
  ...
  private async callProvider(request: ModelRequest): Promise<ModelResponse> {
    try {
      ...
      throw new ProviderError(
        `Provider "${this.config.provider.id}" failed to generate a response.`,
        {
          provider: this.config.provider.id,
          model: this.config.provider.model,
        },
  ...
  private async *streamProvider(
    ...
    try {
      ...
      throw new ProviderError(
        `Provider "${this.config.provider.id}" failed to stream a response.`,
        {
          provider: this.config.provider.id,
          model: this.config.provider.model,
        },
  ...
  private unknownToolResult(toolCall: ToolCall): ToolExecutionResult {
    ...
    return {
      ...
      error: serializeError(
        new ToolExecutionError(`Tool "${toolCall.name}" is not registered.`, {
          tool: toolCall.name,
        }),
      ),
      timing: {
  ...
  private assertImagesSupported(messages: Message[]): void {
    ...
    if (hasImages && !this.config.provider.supportsImages()) {
      throw new ConfigurationError(
        `Provider "${this.config.provider.id}" does not support images.`,
      );
    }
  }
  ...
  private async prepareMessages(inputMessages: Message[], sessionId?: string): Promise<Message[]> {
    ...
    if (this.config.instructions) {
      messages.push(
        systemMessage(this.config.instructions, {

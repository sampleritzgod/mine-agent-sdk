...
test("GuardrailRunner chains modifications through multiple guardrails in order", async () => {
  const runner = new GuardrailRunner([
    { name: "upper", phase: "output", execute: value => ({ allowed: true, value: String(value).toUpperCase() }) },
    { name: "exclaim", phase: "output", execute: value => ({ allowed: true, value: `${String(value)}!` }) },
  ]);
...
test("GuardrailRunner still blocks even when an earlier guardrail modified the value", async () => {
  const runner = new GuardrailRunner([
    { name: "rewrite", phase: "output", execute: value => ({ allowed: true, value: `${String(value)}-rewritten` }) },

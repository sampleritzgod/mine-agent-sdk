...
type ChatResponseFormat = NonNullable<OpenAI.Chat.ChatCompletionCreateParamsNonStreaming["response_format"]>;
...
export class OpenAIProvider implements ModelProvider {
  ...
  private buildParams(request: ModelRequest) {
    return {
      ...
      ...(request.responseFormat ? { response_format: toChatResponseFormat(request.responseFormat) } : {}),
    };
  }
}
...
function toUserContent(message: UserMessage): OpenAI.Chat.ChatCompletionUserMessageParam["content"] {
...
function fromChatToolCalls(calls: OpenAI.Chat.ChatCompletionMessageToolCall[] | undefined): ToolCall[] {
  if (!calls) {
    return [];
  }

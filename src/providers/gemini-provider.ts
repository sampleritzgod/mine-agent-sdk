...
export class GeminiProvider implements ModelProvider {
  ...
  private buildParams(request: ModelRequest): GenerateContentParameters {
    ...
    const config: GenerateContentConfig = {
      ...
      ...(request.tools.length > 0 ? { tools: [{ functionDeclarations: toFunctionDeclarations(request.tools) }] } : {}),
    };
...
function applyResponseFormat(config: GenerateContentConfig, format: ModelResponseFormat | undefined): void {
...
function toGeminiContents(messages: Message[]): { systemInstruction?: string; contents: Content[] } {
  ...
  for (const message of messages) {
    ...
    switch (message.role) {
      ...
      case "assistant":
        ...
        break;
      case "tool": {
        const part: Part = { functionResponse: { name: message.name, response: { output: message.content } } };
 ...
 */
async function* mapGeminiStream(stream: AsyncIterable<GenerateContentResponse>): AsyncGenerator<ModelStreamChunk> {
  let lastId = "";
  ...
  for await (const chunk of stream) {
    ...
    for (const call of chunk.functionCalls ?? []) {
      yield {
        ...
        toolCallDelta: { id: call.id ?? createId("call"), name: call.name ?? "", arguments: call.args ?? {} },
      };
    }
  }

  yield { id: lastId || createId("gemini"), delta: "", done: true, ...(finalUsage ? { usage: finalUsage } : {}) };
}
